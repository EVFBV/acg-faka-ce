...
class Pay extends Base implements PayInterface
{
    public function trade(): PayEntity
    {
        ...
        $totalFee   = (int)round($this->amount * 100);
        ...
        try {
            $response = $this->http()->post(
                ...
                [
                    'json'    => $body,
                    'headers' => $this->buildHeaders($mchId, $apiKey, $body),
                ]
            );
    ...
    private function buildHeaders(string $mchId, string $apiKey, array $body): array
    {
        $timestamp = time();
        ...
        openssl_sign($message, $signature, openssl_get_privatekey($keyPem), OPENSSL_ALGO_SHA256);
